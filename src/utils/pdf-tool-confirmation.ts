...
export function requiresConfirmation(toolName: string): boolean {
  return PDF_TOOLS_REQUIRING_CONFIRMATION.includes(toolName as keyof typeof tools);
}

/**
 * Check if any PDF-related tools are pending confirmation
 */
export function hasPdfToolsPendingConfirmation(messages: any[]): boolean {
 ...
 */
export function getPdfPendingConfirmations(messages: any[]): Array<{
  toolCallId: string;
  toolName: string;
  arguments: any;
}> {
 ...
 */
export function getConfirmationMessage(toolName: string, args: any): string {
  switch (toolName) {
    case "deletePdfFile":
