...
export function CampaignDetails({ campaignId, className }: CampaignDetailsProps) {
  ...
  const handleIndexCampaign = async () => {
    try {
      ...
      const message = error instanceof Error ? error.message : "Failed to trigger indexing";
      toast.error(message);
    } finally {
      setIsIndexing(false);
    }
    ...
    return (
      <Card className={cn("space-y-4", className)}>
        <div className="space-y-2">
          ...
          <p className="text-ob-base-200 text-sm">Loading campaign details...</p>
        </div>
      </Card>
    );
  }
    ...
    <Card className={cn("space-y-4", className)}>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 className="font-medium text-ob-base-300 mb-2">Basic Information</h4>
            <div className="space-y-2 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-ob-base-300 font-mono text-xs">{campaign.campaignId}</span>
              </div>
              <div>
                <span className="text-ob-base-200">Created:</span>
                <span className="ml-2 text-ob-base-300">{formatDate(campaign.createdAt)}</span>
              </div>
              <div>
                <span className="text-ob-base-200">Updated:</span>
                <span className="ml-2 text-ob-base-300">{formatDate(campaign.updatedAt)}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span className="ml-2 text-ob-base-300">{campaign.resources.length}</span>
              </div>
              <div>
                <span className="text-ob-base-200">Resource Types:</span>
                <div className="ml-2 mt-1">
                  {Object.entries(
                    campaign.resources.reduce((acc, resource) => {
