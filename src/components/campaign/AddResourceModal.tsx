...
export function AddResourceModal({
  ...
  useEffect(() => {
    if (file) {
      setResourceType("file");
      setResourceName(file.fileName);
      setResourceId(file.fileKey);
    } else {
      setResourceType("file");
      setResourceName("");
      setResourceId("");
  ...
  const getResourceTypeDescription = (type: ResourceType) => {
    switch (type) {
      case "file":
        return "Upload or link to a file document (rulebooks, adventures, etc.)";
      case "character":
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {resourceType === "file" &&
                  "Enter the file key or URL of the file"}
