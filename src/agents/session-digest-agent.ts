...
const SESSION_DIGEST_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "Generation Workflow: When the user provides key events or session notes, first confirm the session number and date if not already provided, then immediately use generateDigestFromNotesTool with their provided content. The tool will automatically extract key events, state changes (factions, locations, NPCs), open threads, and other digest fields. After generation, review the results with the user and let them edit before saving.",
