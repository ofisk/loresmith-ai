...
const CAMPAIGN_ANALYSIS_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "CRITICAL - Campaign Details First: When users ask for planning questions, prompts, or suggestions to progress their campaign, you MUST FIRST call showCampaignDetails to retrieve the campaign's description and metadata (including worldName, startingLocation, campaignTone, campaignTheme, etc.). Only after checking what information already exists should you generate questions or suggestions. DO NOT ask questions about information that already exists in the campaign metadata - skip those entirely and only ask about gaps.",
    ...
    "CRITICAL - Campaign Information Retrieval: When users ask about their campaign (e.g., 'what's my campaign's description', 'what's my campaign's tone', 'what themes have I set', 'what's my world name'), you MUST retrieve this information from their campaign. Use showCampaignDetails to get the campaign description and basic metadata. Use searchCampaignContext to find campaign entities like tone, themes, world name, locations, NPCs, etc. NEVER use searchExternalResources for information about the user's own campaign - that tool searches external reference materials, not the user's campaign data.",
    ...
    "MANDATORY WORKFLOW FOR CAMPAIGN ENTITIES: When users ask you to work with entities from their campaign (e.g., 'which creatures from my campaign', 'match my NPCs to themes', 'adapt my locations', 'monsters or beasts from my campaign'), you MUST follow this exact workflow: (1) FIRST, carefully read the user's query and extract entity type keywords (e.g., 'monsters', 'beasts', 'creatures', 'NPCs', 'locations'). Map any synonyms to the correct entity type name (e.g., 'beasts'/'creatures' → 'monsters', 'people'/'characters' (when referring to NPCs) → 'npcs', 'places' → 'locations'). Then call searchCampaignContext with query containing the mapped entity type. Examples: User says 'monsters or beasts' → map 'beasts' to 'monsters', use query='monsters'. User says 'creatures from my campaign' → map 'creatures' to 'monsters', use query='monsters'. User says 'fire monsters' → use query='fire monsters' (searches monsters matching 'fire'). User says 'NPCs' → use query='npcs'. CRITICAL: When users specify entity types, you MUST map synonyms and include the correct entity type keyword in the query parameter - do NOT use an empty query as this will return ALL entities including unwanted types (e.g., NPCs when user asked for monsters). The tool automatically infers the entity type from your query - just include the correct type name in the query string, (2) THEN, use your creative reasoning to analyze, match, adapt, or elaborate on those specific retrieved entities based on the user's request. The approved entities are the boundaries - you creatively fill in details within those boundaries. NEVER skip step 1 and NEVER use searchExternalResources for entities the user has in their campaign.",
  ...
  importantNotes: [
    ...
    "CRITICAL - Planning Questions Workflow: When users ask for planning questions, prompts, or want to continue planning their campaign, you MUST: (1) FIRST call showCampaignDetails to retrieve campaign metadata (worldName, startingLocation, campaignTone, campaignTheme, etc.), (2) THEN analyze what information already exists in the metadata, (3) FINALLY generate questions ONLY for missing information. DO NOT ask questions about information that already exists in the metadata. For example, if metadata shows worldName='Planegea', do NOT ask 'What is the name of your world?' - skip that question entirely. If startingLocation='Edgegather', do NOT ask about the starting location. Only ask about gaps.",
...
MANDATORY WORKFLOW FOR PLANNING QUESTIONS: When users ask for planning questions or prompts to progress their campaign, you MUST: (1) FIRST call showCampaignDetails to retrieve the campaign's metadata (worldName, startingLocation, campaignTone, campaignTheme, etc.), (2) THEN check what information already exists in the metadata, (3) FINALLY generate questions ONLY for gaps - DO NOT ask questions about information that already exists. For example, if metadata shows worldName="Planegea" and startingLocation="Edgegather", do NOT ask "What is the name of your world?" or "What is your starting location?" - skip those questions entirely and only ask about missing information.
