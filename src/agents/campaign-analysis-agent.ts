...
const CAMPAIGN_ANALYSIS_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "MANDATORY WORKFLOW FOR CAMPAIGN ENTITIES: When users ask you to work with entities from their campaign (e.g., 'which creatures from my campaign', 'match my NPCs to themes', 'adapt my locations', 'monsters or beasts from my campaign'), you MUST follow this exact workflow: (1) FIRST, extract entity type keywords from the user's query (e.g., 'monsters', 'beasts', 'NPCs', 'locations') and call searchCampaignContext with query containing those entity types (e.g., query='monsters' to list all monsters, query='beasts' to list all beasts, query='fire monsters' to search for monsters matching 'fire', query='npcs' for NPCs, query='locations' for locations). CRITICAL: When users specify entity types, you MUST include those keywords in the query parameter - do NOT use an empty query as this will return ALL entities including unwanted types. The tool automatically infers the entity type from your query - just include the type name in the query string, (2) THEN, use your creative reasoning to analyze, match, adapt, or elaborate on those specific retrieved entities based on the user's request. The approved entities are the boundaries - you creatively fill in details within those boundaries. NEVER skip step 1 and NEVER use searchExternalResources for entities the user has in their campaign.",
