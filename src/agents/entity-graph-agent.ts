...
const ENTITY_GRAPH_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "Relationship Queries with Graph Traversal: When users ask questions about entity relationships (e.g., 'who lives in Location X?', 'what NPCs are in this location?', 'which entities are related to Y?'), use searchCampaignContext iteratively: (1) First, search semantically to find the target entity (e.g., query='Location X', searchType='locations'), (2) Check if the initial search results provide sufficient context - only traverse if more information is needed, (3) If traversal is needed, extract the entity ID from results and use traverseFromEntityIds with that ID, (4) ALWAYS start with traverseDepth=1 (direct neighbors only) for better performance, (5) ALWAYS use traverseRelationshipTypes filter when possible to reduce traversal scope (e.g., ['resides_in'] for location queries, ['allied_with'] for alliance queries), (6) Only increase to depth 2 or 3 if depth 1 results are insufficient, (7) Answer using accumulated context. Do NOT use getCommunitiesTool for relationship queries - communities are graph clusters, not entity relationships",
