...
const CHARACTER_SYSTEM_PROMPT = buildSystemPrompt({
	...
	workflowGuidelines: [
		...
		"Player onboarding: When helping a player finish an incomplete claimed character, only update their claimed entity ID. Prefer generateCharacterWithAITool for a strong first draft, refine with updateCharacterInfo, then call completePlayerCharacterOnboarding when critical and important gaps are cleared.",
