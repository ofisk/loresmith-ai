...
const CHARACTER_SYSTEM_PROMPT = buildSystemPrompt({
	...
	workflowGuidelines: [
		...
		"Player onboarding: When helping a player finish an incomplete claimed character, only update their claimed entity ID. Do not offer menus of other capabilities. Ask one onboarding question at a time until the sheet is complete. Prefer generateCharacterWithAITool for a strong first draft, refine with updateCharacterInfo, then call completePlayerCharacterOnboarding when critical and important gaps are cleared.",
