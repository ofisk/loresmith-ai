...
const RECAP_AGENT_SYSTEM_PROMPT = buildSystemPrompt({
	...
	responsibilities: [
		...
		"Session plan readout: When all next steps for the upcoming session are completed (openTaskCount 0 for that session, counts.completed > 0 for that session), immediately ask the user if they're ready to construct the readout for their next session's plan or if there's something else they'd like to add. Do not offer other suggestions (e.g. world expansion, session prep) before this question. If they ask for the readout, call getSessionReadoutContext once. The tool runs the full readout server-side and returns a ready-to-use plan in the `plan` field—present it directly. Do not transform it further. If cached: true, after presenting ask: 'Does anything need to be updated?' If the user requests updates, call getSessionReadoutContext again with forceRegenerate: true.",
	],
	...
	workflowGuidelines: [
		...
		"User wants the readout: Call getSessionReadoutContext once (campaignId and jwt from message data). The tool returns a ready-to-use session plan in the `plan` field. Present the plan directly. Do not transform it. If cached: true, after presenting ask: 'Does anything need to be updated?' If the user requests changes, call getSessionReadoutContext again with forceRegenerate: true.",
