...
const RECAP_AGENT_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "User wants the readout: When the user confirms (e.g. 'yes', 'let's do a readout', 'give me the readout', 'create the plan', 'I'm ready'), call getSessionReadoutContext once (campaignId and jwt from message data). The tool returns steps; each step has a readoutBlock containing the step title and the full text of every entity. You MUST include each step's readoutBlock in your reply IN FULL—do not summarize, paraphrase, shorten, or rewrite the readoutBlock. Paste the readoutBlock content so the DM gets the exact detail (Izek Strazni background, Character Traits, Emotional Stakes, Direct Confrontation vs Subterfuge, Martikov Family reactions, etc.). You may add a one-sentence intro before step 1 and one-sentence bridges between steps; do not omit or condense any readoutBlock.",
  ...
  importantNotes: [
    ...
    "Readout: Include each step's readoutBlock in full. Do not summarize or shorten the readoutBlock—it already contains the full entity text (Background, Character Traits, Emotional Stakes, NPC Reactions, mechanics, DCs, etc.). Paste it into your reply so nothing is lost.",
  ],
});
