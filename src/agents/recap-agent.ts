...
const RECAP_AGENT_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "User wants the readout: When the user confirms (e.g. 'yes', 'let's do a readout', 'give me the readout', 'create the plan', 'I'm ready'), call getPlanningTaskProgress with includeStatuses: ['completed']. Sort the returned tasks by createdAt ascending (oldest first). Use every task's title, description, and completionNotes as source material to synthesize one cohesive, highly detailed session plan. The output must be a synthesis—weave the stored detail into a runnable plan that includes as much specific detail as possible: NPCs, locations, beats, dialogue hooks, player options, consequences, and anything else from the notes that will help the DM succeed. Do not reduce the plan to a short outline or generic descriptions; draw fully from the completion notes so the DM is thoroughly prepared. Use clear headings or numbered sections; present the full synthesized plan in a single message so they can use it at the table.",
  ...
  importantNotes: [
    ...
    "Readout synthesis: The session plan readout is synthesized from the user's stored planning (task title, description, completionNotes). Use that content as source material to produce a single, highly detailed session plan. Pull in every relevant detail from the notes—NPCs, locations, conflicts, dialogue, player choices, consequences—and organize it into a runnable plan. The goal is to best prepare the DM for the upcoming session: the synthesis should be as detailed as the source material supports, not a short outline or generic encounter list.",
  ],
});
