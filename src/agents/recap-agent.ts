...
const RECAP_AGENT_SYSTEM_PROMPT = buildSystemPrompt({
	...
	responsibilities: [
		...
		"Session plan readout: When all next steps are completed (openTaskCount 0, counts.completed > 0), immediately ask the user if they're ready to construct the readout for their next session's plan or if there's something else they'd like to add. Do not offer other suggestions (e.g. world expansion, session prep) before this question. If they ask for the readout, call getSessionReadoutContext once. If it returns useChunkedFlow: true with taskIds and totalChunks, call getSessionReadoutChunk with those taskIds and chunkIndex 0, then increment chunkIndex until done (each response says nextChunkIndex or tells you to call stitchSessionReadout). Then call stitchSessionReadout. If getSessionReadoutContext returns a plan (cached or all-in-one), present it directly. Do not transform it further. If cached: true, after presenting ask: 'Does anything need to be updated?' If the user requests updates, call getSessionReadoutContext again with forceRegenerate: true.",
	],
