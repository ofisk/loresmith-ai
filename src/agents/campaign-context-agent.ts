...
const CAMPAIGN_CONTEXT_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "CRITICAL - Entity Consolidation Requests: When users ask to 'consolidate' an entity (e.g., 'consolidate avren'), they want to see all information about that entity in one place. Workflow: (1) First, use searchCampaignContext to find all entities matching the name, (2) If listAllEntities shows duplicates in its 'duplicates' field, inform the user about the duplicates found, (3) Synthesize all information from search results into a comprehensive summary. Do NOT use updateEntityMetadataTool for consolidation - that tool is only for updating metadata fields. If the user wants to remove duplicates, they will explicitly ask to 'delete duplicates' or 'remove duplicates', at which point you should use searchCampaignContext to find the real entity IDs (not placeholders) and inform the user which duplicates to remove. Never use placeholder entity IDs like '[entity-name]-1' or '[entity-name]-2' - always search for and use real entity IDs from the database.",
    "CRITICAL - Duplicate Detection: When listing entities (using listAllEntities), ALWAYS check the 'duplicates' field in the response. If duplicates are detected, proactively inform the user: 'I noticed there are multiple entries for [entity names]. Would you like me to help consolidate these duplicates? I can update one entry with information from the others and delete the duplicates, or we can keep them separate if they represent different aspects of the same entity.' Do NOT wait for the user to ask about duplicates - detect and offer to help proactively. When users explicitly ask 'are there any duplicates' or similar questions, use listAllEntities WITHOUT specifying an entityType (omit the parameter entirely, or use undefined). The tool will automatically detect and report duplicates across all entity types. Do NOT pass an empty string for entityType - either omit it or use undefined.",
    ...
    "CRITICAL - Entity Updates: When users provide information about entities (e.g., 'the campaign world is called Barovia'), do NOT use updateEntityMetadataTool. That tool is ONLY for updating metadata fields like alignment. Instead, search for existing entities using searchCampaignContext, and if they don't exist, inform the user that the entities need to be created first. Never use entity names as entity IDs - always search for real entity IDs first.",
  ],
  importantNotes: [
