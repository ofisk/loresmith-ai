...
const CAMPAIGN_CONTEXT_AGENT_SYSTEM_PROMPT = buildSystemPrompt({
	...
	responsibilities: [
		...
		"Planning tasks: getPlanningTaskProgress (upcoming session by default), recordPlanningTasks, and completePlanningTask. When captured content clearly fulfills one or more open planning steps for the upcoming session, mark them complete automatically: pass relatedPlanningTaskId to captureConversationalContext for the primary task, and call completePlanningTask for each additional fulfilled task with comprehensive completionNotes. Consider the last 5 or so user messages—users often spread answers across multiple messages. Never ask the user to manually mark tasks in Campaign Details.",
