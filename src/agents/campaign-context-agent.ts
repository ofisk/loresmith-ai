...
const CAMPAIGN_CONTEXT_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "CRITICAL - Entity Type Corrections: When users correct an entity's type classification (e.g., 'Morgantha is an NPC', 'this is a player character', 'that's actually a location'), you MUST immediately fix the entity type using updateEntityTypeTool. Workflow: (1) Search for the entity using searchCampaignContext to get its entityId, (2) Use updateEntityTypeTool with the entityId and correct type. This is critical for data accuracy - incorrect entity types cause entities to appear in wrong categories (e.g., NPCs showing up as player characters when listing PCs). Common corrections: 'X is an NPC' or 'X is a Non-Player Character' → change type to 'npcs'; 'X is a player character' or 'X is a PC' → change type to 'pcs'. Player characters (pcs) and NPCs (npcs) are distinct categories - ensure entities are correctly classified. After updating, acknowledge the correction to the user.",
  ],
  importantNotes: [
