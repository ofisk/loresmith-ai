...
const CAMPAIGN_CONTEXT_SYSTEM_PROMPT = buildSystemPrompt({
  ...
  workflowGuidelines: [
    ...
    "Planning Task Tracking and Next Steps: When you provide concrete, actionable next steps (for example, 'Prepare Baba Lysaga's character and motivations' or 'Sketch the starting town map'), you MUST treat them as planning tasks. Before suggesting new next steps, call getPlanningTaskProgress so you can reference what the user has already completed or started and avoid repeating completed tasks. After deciding on new next steps, call recordPlanningTasks with a structured list of tasks (titles and optional descriptions) so they are saved for the user. When the user clearly works on a specific recorded task and you capture that context using captureConversationalContext, pass relatedPlanningTaskId so the system can mark that task as completed.",
  ],
  importantNotes: [
