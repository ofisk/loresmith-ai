...
const getCampaignSystemPrompt = () =>
  buildSystemPrompt({
    ...
    workflowGuidelines: [
      ...
      "CRITICAL - MANDATORY TOOL USAGE FOR CAMPAIGN ENTITIES: When users ask about entities (monsters, beasts, creatures, NPCs, locations, factions, hooks, etc.) from their campaign, or ask for suggestions about campaign content, you MUST call searchCampaignContext ONCE BEFORE generating any response. This is mandatory - you CANNOT respond without calling the tool first. CRITICAL - CALL THE TOOL ONLY ONCE: When users mention multiple synonyms (e.g., 'monsters or beasts', 'beasts or creatures'), these are synonyms for the SAME entity type. You MUST map all synonyms to the correct entity type name and call searchCampaignContext ONCE with that mapped type. DO NOT call the tool multiple times with different synonyms. Entity type mapping: 'beasts'/'creatures' → 'monsters', 'people'/'characters' (when referring to NPCs) → 'npcs', 'places' → 'locations'. Examples: User says 'monsters or beasts from my campaign' → call ONCE with query='monsters' (NOT twice with 'monsters' and 'beasts'). User says 'beasts or creatures' → call ONCE with query='monsters'. User says 'monsters from my campaign' → call ONCE with query='monsters'. DO NOT use empty queries when entity types are specified. Base responses ONLY on approved entities found in the campaign. If no results are found, explain what you searched for and ask if the user wants to create new content. Approved entities define the boundaries - use creative reasoning within those boundaries to interpret and match entities to user requests.",
