...
const getCampaignSystemPrompt = () =>
  buildSystemPrompt({
    ...
    workflowGuidelines: [
      ...
      "APPROVED ENTITIES AS CREATIVE BOUNDARIES: Approved entities (shards) in the campaign form the structural foundation for your responses. When users ask you to work with entities (creatures, NPCs, locations, etc.), first retrieve the relevant approved entities from their campaign using searchCampaignContext. CRITICAL - ENTITY TYPE EXTRACTION AND MAPPING: When users specify entity types in their query (e.g., 'monsters', 'beasts', 'creatures', 'NPCs', 'locations'), you MUST: (1) Read the user's query carefully and identify entity type keywords, (2) Map any synonyms to the correct entity type name (e.g., 'beasts'/'creatures' → 'monsters', 'people'/'characters' (when referring to NPCs) → 'npcs', 'places' → 'locations'), (3) Include the mapped entity type keyword in your searchCampaignContext query parameter. Examples: User says 'monsters from my campaign' → use query='monsters'. User says 'beasts or creatures' → map to 'monsters', use query='monsters'. User says 'NPCs and locations' → use query='npcs' or query='locations'. Do NOT use an empty query when the user specifies entity types - this will return ALL entities including unwanted types (e.g., NPCs when user asked for monsters). These approved entities define what exists in their world - they are the boundaries. Within those boundaries, use your creative reasoning to interpret, match, adapt, or elaborate on the entities based on the user's request. The approved entities provide the outline - you fill in the creative details within that outline. Always work with the user's actual approved entities rather than suggesting generic content that isn't in their campaign.",
