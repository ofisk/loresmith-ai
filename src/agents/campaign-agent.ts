...
const getCampaignSystemPrompt = () =>
  buildSystemPrompt({
    ...
    importantNotes: [
      ...
      "CRITICAL - Entity Type Corrections: When users correct an entity's type classification (e.g., '[entity name] is an NPC', 'update [entity] to be an npc, not a pc', 'this is a player character'), you MUST immediately fix the entity type using updateEntityTypeTool. Workflow: (1) Search for the entity using searchCampaignContext to get its entityId - the search results will include an 'id' field for each entity, (2) Extract the actual entityId from the search results (NOT a placeholder like 'entity-id' or '[name]-id'), (3) Use updateEntityTypeTool with the REAL entityId from the search results and the correct type. The tool will automatically update ALL entities with the same name to ensure consistency. Common corrections: '[entity] is an NPC' or '[entity] is a Non-Player Character' → change type to 'npcs'; '[entity] is a player character' or '[entity] is a PC' → change type to 'pcs'. After updating, verify the change by re-querying to confirm it no longer appears in the wrong category.",
      "CRITICAL - Duplicate Consolidation: When users ask to consolidate or remove duplicates (e.g., 'consolidate duplicates', 'remove duplicates', 'collapse these', 'delete the duplicates', 'proceed with removing duplicates'), you MUST: (1) First use searchCampaignContext to search for entities with duplicate names (e.g., query='[entity name]') OR use listAllEntities if available to get the full list and check the 'duplicates' field in the response, (2) Extract the REAL entityIds from the search/list results (NOT placeholders like '[entity-name]-duplicate-id' or '[name]-id' - use the actual 'id' field from the results), (3) Identify which entity should be kept (usually the one with the most complete information or the most recent), (4) For each duplicate to delete, use deleteEntityTool with the REAL entityId from the results. If the user doesn't specify which to keep, ask them or choose the one with the most complete metadata/content. Always confirm which entities you're deleting before deleting them. After deletion, verify by re-querying to confirm duplicates are gone. NEVER use placeholder IDs - always extract real IDs from search results.",
    ],
