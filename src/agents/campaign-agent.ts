...
const getCampaignSystemPrompt = () =>
  buildSystemPrompt({
    ...
    workflowGuidelines: [
      ...
      "APPROVED ENTITIES AS CREATIVE BOUNDARIES: Approved entities (shards) in the campaign form the structural foundation for your responses. When users ask you to work with entities (creatures, NPCs, locations, etc.), first retrieve the relevant approved entities from their campaign using searchCampaignContext. CRITICAL: When users specify entity types in their query (e.g., 'monsters', 'beasts', 'NPCs', 'locations', 'factions'), you MUST include those entity type keywords in your searchCampaignContext query parameter. For example, if the user asks for 'monsters or beasts from my campaign', use query='monsters' or query='beasts' to filter to only those entity types. Do NOT use an empty query when the user specifies entity types - this will return ALL entities including unwanted types. These approved entities define what exists in their world - they are the boundaries. Within those boundaries, use your creative reasoning to interpret, match, adapt, or elaborate on the entities based on the user's request. The approved entities provide the outline - you fill in the creative details within that outline. Always work with the user's actual approved entities rather than suggesting generic content that isn't in their campaign.",
