...
export const generateEncounterTool = tool({
	...
	execute: async (
		...
		try {
			const ctxResult = resolveToolContext(options, toolCallId, {
				notAvailable: "Environment not available",
				detail: "Direct database access is required for encounter generation.",
			});
			if (!ctxResult.ok) {
			...
			const { env, daoFactory } = ctxResult.context;
