...
function fireFileProcessingTelemetry(
	env: { DB?: D1Database },
	durationMs: number,
	...
	void new TelemetryService(new TelemetryDAO(env.DB))
		.recordFileProcessingDuration(durationMs, { metadata })
		.catch(() => {});
}
...
export class SyncQueueService {
	...
	static async processSyncQueue(
		...
		for (const item of queueItems) {
			...
			try {
				...
				if (!file) {
					log.warn("File not found in R2, removing from queue", {
						file_key: item.file_key,
					});
				...
				if (!dbMetadata) {
					log.warn("File metadata not found, removing from queue", {
						file_key: item.file_key,
					});
					await fileDAO.removeFromSyncQueue(item.file_key);
					continue;
				...
				if (hasChunks) {
					log.info("Processing file chunks", {
						file_key: item.file_key,
						file_name: item.file_name,
					});
					// Process file chunks
					...
					if (mergeResult.allComplete && mergeResult.allSuccessful) {
						...
						log.error("Chunk processing failed: not all chunks successful", {
							file_key: item.file_key,
							file_name: item.file_name,
							stats: mergeResult.stats,
				...
				log.error("Queue item error", error, {
					file_key: item.file_key,
					file_name: item.file_name,
					errorMessage,
					errorStack,
	...
	private static async processFileChunks(
		...
		const log = createLogger(env, "[SyncQueueChunks]");
		const fileDAO = new FileDAO(env.DB);
		const chunkedService = new ChunkedProcessingService(env);
		...
		log.info("processFileChunks start", {
			fileKey,
			fileName,
			fileSizeMB: Math.round(fileSizeMB * 100) / 100,
			pendingChunks: pendingChunks.length,
			contentType,
			shouldSkipFullBuffer,
			usePerChunkFetch,
		});
		...
		if (!shouldSkipFullBuffer) {
			...
			try {
				...
				log.warn("Full buffer load failed, using per-chunk fetch", {
					fileKey,
					error:
			...
			try {
				...
				if (hasByteRange) {
					...
					if (isPdfChunk && fileSize > 0) {
						log.info("Processing PDF chunk via R2 range transport", {
							fileKey,
							chunkId: chunk.id,
				...
				log.error("Chunk processing error", chunkError, {
					fileKey,
					fileName,
					chunkId: chunk.id,
					chunkIndex: chunk.chunkIndex,
