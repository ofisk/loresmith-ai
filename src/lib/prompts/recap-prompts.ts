...
SAVING NEXT STEPS: When you suggest next steps, they must be saved so they appear in Campaign Details. (1) FIRST call getPlanningTaskProgress (scoped to the upcoming session by default). If there are open (pending/in_progress) tasks for that session, present those with their session number and tell the user they can view them in Campaign Details under the Next steps tab—do NOT call recordPlanningTasks. (2) If there are NO open tasks for the upcoming session, after suggesting 3-5 next steps you MUST call recordPlanningTasks with those tasks (each with "title" and optional "description"; they are pinned to the upcoming session automatically). Campaign ID and JWT are injected automatically—you only need to pass the tasks array. Do not say "these have been saved" in your reply without having called recordPlanningTasks; after the tool succeeds, tell the user: "These have been saved to your campaign. You can view and manage them in Campaign Details under the Next steps tab."
...
SAVING NEXT STEPS: When you suggest next steps, they must be saved so they appear in Campaign Details. (1) FIRST call getPlanningTaskProgress (scoped to the upcoming session by default). If there are open (pending/in_progress) tasks for that session, present those with their session number and tell the user they can view them in Campaign Details under the Next steps tab—do NOT call recordPlanningTasks. (2) If there are NO open tasks for the upcoming session, after suggesting 2-3 next steps you MUST call recordPlanningTasks with those tasks (each with "title" and optional "description"; they are pinned to the upcoming session automatically). Campaign ID and JWT are injected automatically—you only need to pass the tasks array. Do not say "these have been saved" in your reply without having called recordPlanningTasks; after the tool succeeds, tell the user: "These have been saved to your campaign. You can view and manage them in Campaign Details under the Next steps tab."
