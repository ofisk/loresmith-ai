...
describe("Tool Structure Validation", () => {
  ...
  it("has properly structured tool definitions", () => {
    ...
    for (const toolName of toolNames) {
      ...
      expect((tool as { description: string }).description.length).toBeGreaterThan(0);
    }
  });
});
...
describe("PDF Tools Structure", () => {
  ...
  it("has PDF upload URL generation tool", () => {
    const uploadUrlTool = (tools as Record<string, unknown>).generatePdfUploadUrl;
    expect(uploadUrlTool).toBeDefined();
    expect((uploadUrlTool as { description: string }).description).toContain("upload");
    expect((uploadUrlTool as { description: string }).description).toContain("PDF");
  ...
  it("has PDF authentication status tool", () => {
    ...
    expect((authTool as { description: string }).description).toContain("authenticated");
...
describe("Scheduling Tools Structure", () => {
  ...
  it("has task scheduling tool", () => {
    ...
    expect((scheduleTool as { description: string }).description).toContain("schedule");
    expect((scheduleTool as { description: string }).description).toContain("task");
  ...
  it("has scheduled tasks listing tool", () => {
    ...
    expect((listTasksTool as { description: string }).description).toContain("scheduled");
    expect((listTasksTool as { description: string }).description).toContain("tasks");
