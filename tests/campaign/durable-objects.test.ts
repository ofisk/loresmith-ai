...
describe("Campaign Durable Objects", () => {
  ...
  describe("CampaignManager", () => {
    it("should store campaign data in KV", async () => {
      ...
      await mockKV.put(`user:demo-user:campaign:test-campaign`, JSON.stringify(mockCampaign));
    ...
    it("should retrieve campaign data from KV", async () => {
      ...
      const campaignData = await mockKV.get(`user:demo-user:campaign:test-campaign`);
      const parsedCampaign = JSON.parse(campaignData as string) as CampaignData;

      expect(mockKV.get).toHaveBeenCalledWith(`user:demo-user:campaign:test-campaign`);
    ...
    it("should handle missing campaign data", async () => {
      ...
      const campaignData = await mockKV.get(`user:demo-user:campaign:nonexistent`);

      expect(mockKV.get).toHaveBeenCalledWith(`user:demo-user:campaign:nonexistent`);
    ...
    it("should list all campaigns for a user", async () => {
      ...
      const listResult = await mockKV.list({ prefix: "user:demo-user:campaign:" });
      
      expect(mockKV.list).toHaveBeenCalledWith({ prefix: "user:demo-user:campaign:" });
    ...
    it("should delete campaign data", async () => {
      ...
      expect(mockKV.delete).toHaveBeenCalledWith(`user:demo-user:campaign:test-campaign`);
    });
  });
  ...
  describe("Campaign Resource Management", () => {
    it("should add resource to campaign", async () => {
      ...
      await mockKV.put(`user:demo-user:campaign:test-campaign`, JSON.stringify(updatedCampaign));
    ...
    it("should remove resource from campaign", async () => {
      ...
      const updatedCampaign = {
        ...existingCampaign,
        resources: existingCampaign.resources.filter(r => r.id !== resourceToRemove),
        updatedAt: new Date().toISOString(),
      };
  ...
  describe("Campaign Indexing", () => {
    it("should trigger indexing for campaign with resources", async () => {
      const mockCampaign = createMockCampaign({
        ...
        name: "Test Campaign",
        resources: [
          createMockResource({ id: "pdf-1", name: "Document.pdf", type: "pdf" }),
          createMockResource({ id: "pdf-2", name: "Adventure.pdf", type: "pdf" }),
        ],
      });
