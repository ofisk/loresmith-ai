...
import { createTestEnv, createMockCampaign, createMockResource } from "./testUtils";
...
describe("Campaign API Endpoints", () => {
  ...
  describe("POST /campaign/:id/index", () => {
    it("should trigger indexing for a valid campaign", async () => {
      ...
      const request = new Request("http://example.com/campaign/test-campaign-123/index", {
        method: "POST",
      });
    ...
    it("should return 404 when campaign is not found", async () => {
      ...
      const request = new Request("http://example.com/campaign/nonexistent/index", {
        method: "POST",
      });
  ...
  describe("POST /campaigns/:id/resource", () => {
    it("should add a resource to a campaign", async () => {
      ...
      const env = createTestEnv([], mockCampaign);
      const request = new Request("http://example.com/campaigns/test-campaign/resource", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          type: "pdf",
  ...
  describe("DELETE /campaigns/:id/resource/:resourceId", () => {
    it("should remove a resource from a campaign", async () => {
      ...
      const env = createTestEnv([], mockCampaign);
      const request = new Request("http://example.com/campaigns/test-campaign/resource/pdf-1", {
        method: "DELETE",
      });
    ...
    it("should return 404 when resource is not found in campaign", async () => {
      ...
      const env = createTestEnv([], mockCampaign);
      const request = new Request("http://example.com/campaigns/test-campaign/resource/nonexistent", {
        method: "DELETE",
      });
