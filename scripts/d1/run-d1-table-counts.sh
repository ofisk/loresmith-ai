#!/usr/bin/env bash
# Remote D1 row counts (see scripts/d1/d1-cleanup-candidate-counts.sql).
#
...
# Optional FTS5 shadow tables: scripts/d1/d1-fts-shadow-counts.sql (run only if those tables exist).
set -euo pipefail
cd "$(dirname "$0")/../.."
...
SQL_F="scripts/d1/d1-cleanup-candidate-counts.sql"
